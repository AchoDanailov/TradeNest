...
export default async function getProductDetailsModalTemplate(
    productId: string,
                            ...
                            ${productDetails.ownerName != null
                                    ? html`
                                        <div class="row manage-approval-row mt-4">
                                            <div class="col-12">
                                                <form @submit=${async (event: Event) => onModifyApproval(event, productDetails, context)}>
...
function errTemplate(productDetailsModalId: string): TemplateResult {
    return html`
...
async function onModifyApproval(
    event: Event,
    productDetails: ProductDetails,
    context: TableContext
): Promise<void> {
    ...
    if(newApprovalStatus === productDetails.approvalDecision.approvalStatus) {
        (event.currentTarget as HTMLFormElement)
            .querySelector<HTMLDivElement>("div.approval-validation-section")!
...
function onStatusChangeValidation(event: Event) {
    const validationSection = (event.currentTarget as HTMLDivElement)
...
function approvalDecisionJustificationValidation(event: Event) {
...
function getStatusBadgeClass(status: ProductsApprovalStatus) {
    switch (status) {
...
function formatStatus(status: ProductsApprovalStatus) {
    switch (status) {
